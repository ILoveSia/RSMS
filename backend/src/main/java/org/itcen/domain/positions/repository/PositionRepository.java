 ...
 * 
 * SOLID 원칙:
 * - Single Responsibility: 직책 데이터 액세스만 담당
...
public interface PositionRepository extends JpaRepository<Position, Long> {
    ...
    Page<Position> findBySearchConditions(
            @Param("positionsId") Long positionsId,
    ...
    /**
     * 직책 현황 목록 조회 (Native Query)
     */
    @Query(value = """
        SELECT
            p.positions_id AS positionsId,
            p.positions_nm AS positionsNm,
            COALESCE(cc_write.code_name, p.write_dept_cd) AS writeDeptNm,
            (
                SELECT STRING_AGG(COALESCE(cc_owner.code_name, pod.owner_dept_cd), ', ')
                FROM positions_owner_dept pod
                LEFT JOIN common_code cc_owner ON pod.owner_dept_cd = cc_owner.code AND cc_owner.group_code = 'DEPT'
        ...
        FROM
            positions p
        LEFT JOIN
            common_code cc_write ON p.write_dept_cd = cc_write.code AND cc_write.group_code = 'DEPT'
        ORDER BY
            p.created_at DESC
    """, nativeQuery = true)
