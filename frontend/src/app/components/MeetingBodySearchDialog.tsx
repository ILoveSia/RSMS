...
import { meetingBodyApi } from '../services/api';
...
const MeetingBodySearchDialog: React.FC<MeetingBodySearchDialogProps> = ({
  ...
  // 선택된 항목 상태
  const [selectedMeetingBody, setSelectedMeetingBody] = useState<MeetingBodySearchResult | null>(null);
  ...
  const getCodeName = (groupCode: string, code: string): string => {
    ...
    for (const possibleGroup of possibleGroupCodes) {
      foundCode = codes.find(c => c.groupCode === possibleGroup && c.code === code && c.useYn === 'Y');
      if (foundCode) {
    ...
    if (!foundCode) {
      ...
      console.log(`🔍 ${groupCode} 관련 그룹들:`, codes.filter(c => c.groupCode.includes(groupCode.replace(/\d+$/, ''))));
    }
  ...
  useEffect(() => {
    ...
    console.log('🔍 GUBUN01 공통코드:', codes.filter(c => c.groupCode === 'GUBUN01'));
    console.log('🔍 PID03 공통코드:', codes.filter(c => c.groupCode === 'PID03'));
  }, [allCodes]);
  ...
  const loadMeetingBodies = async () => {
    ...
    try {
      // 전체 회의체 목록 조회 API 호출
      const meetingBodyList: MeetingBody[] = await meetingBodyApi.getAll();
  ...
  const handleSearch = () => {
    ...
    const filtered = meetingBodies.filter(meeting =>
                    ...
                    <ListItem disablePadding>
                      <ListItemButton
                        ...
                        sx={{
                          ...
                          '&:hover': {
                            backgroundColor: selectedMeetingBody?.id === meeting.id 
                              ? '#1565c0' 
                              : 'rgba(0, 0, 0, 0.04)',
                          }
                        }}
