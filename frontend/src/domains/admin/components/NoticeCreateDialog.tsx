...
interface NoticeCreateDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: { category?: string; title: string; content?: string; is_public?: boolean; pinned?: boolean }) => Promise<void> | void;
  loading?: boolean;
}
...
const NoticeCreateDialog: React.FC<NoticeCreateDialogProps> = ({ open, onClose, onSubmit, loading }) => {
  ...
  return (
    <BaseDialog
      ...
      mode='create'
      title='공지 등록'
      onSave={async () => { await onSubmit({ category: category || undefined, title, content: content || undefined, is_public: isPublic, pinned }); }}
    ...
    >
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 2 }}>
          <FormControlLabel control={<Switch checked={isPublic} onChange={e => setIsPublic(e.target.checked)} />} label='공개' />
