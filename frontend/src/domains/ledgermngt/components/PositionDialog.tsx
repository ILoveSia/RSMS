...
import MeetingBodySearchDialog, { type MeetingBodySearchResult } from '../../../app/components/MeetingBodySearchDialog';
...
export interface PositionData {
  ...
  meetings: Array<{ meetingBodyId: string; meetingBodyName: string; memberGubun: string; meetingPeriod: string; deliberationContent: string; }>;
...
const PositionDialog: React.FC<PositionDialogProps> = ({
  ...
  const getDeptCodes = () => {
    ...
    return codes.filter(code => 
      code.groupCode === 'DEPT' && code.useYn === 'Y'
  ...
  const getMebGubunCodes = () => {
    const codes = getCodesArray();
    return codes.filter(code => 
      code.groupCode === 'MEB_GUBUN' && code.useYn === 'Y'
    ).sort((a, b) => a.sortOrder - b.sortOrder);
  };
  ...
  const [meetings, setMeetings] = useState<MeetingData[]>([
    { id: '1', meetingBodyId: '', meetingBodyName: '', memberGubun: '', meetingPeriod: '', deliberationContent: '' }
  ]);
  ...
  useEffect(() => {
    const fetchPositionDetails = async (id: number) => {
      ...
      try {
        ...
        if (response && response.success) {
          ...
          setOwnerDepts(ownerDeptsData.length > 0 ? ownerDeptsData.map((d, i) => ({ id: String(i + 1), ...d })) : [{ id: '1', deptCode: '', deptName: '' }]);
          setMeetings(meetingsData.length > 0 ? meetingsData.map((m, i) => ({ 
            id: String(i + 1), 
            ...m,
            memberGubun: m.memberGubun === 'GUBUN01' ? 'MEG01' : m.memberGubun // 데이터 임시 보정
          })) : [{ id: '1', meetingBodyId: '', meetingBodyName: '', memberGubun: '', meetingPeriod: '', deliberationContent: '' }]);
          setManagers(managersData.length > 0 ? managersData.map((m, i) => ({ id: String(i + 1), ...m })) : [{ id: '1', empNo: '', empName: '', position: '' }]);
        } else {
    ...
    if (open) {
      ...
      if ((mode === 'edit' || mode === 'view') && positionId) {
        ...
        setMeetings([{ id: '1', meetingBodyId: '', meetingBodyName: '', memberGubun: '', meetingPeriod: '', deliberationContent: '' }]);
  ...
  const handleOwnerDeptChange = (id: string, field: keyof OwnerDept, value: string) => {
    setOwnerDepts(prev => prev.map(dept => 
  ...
  const addMeeting = () => {
    const newId = Date.now().toString();
    setMeetings(prev => [...prev, { id: newId, meetingBodyId: '', meetingBodyName: '', memberGubun: '', meetingPeriod: '', deliberationContent: '' }]);
  };
              ...
              <Table size="small">
                <TableHead>
                  <TableRow sx={{ backgroundColor: '#f5f5f5' }}>
                    ...
                    {mode !== 'view' && <TableCell sx={{ fontWeight: 'bold', width: 80 }}>작업</TableCell>}
                  </TableRow>
                </TableHead>
                <TableBody>
                  {ownerDepts.map((dept) => (
                      ...
                      <TableCell>
                        <FormControl fullWidth size="small">
                          <Select
                            value={dept.deptCode}
                            onChange={(e) => {
                              const selectedDept = getDeptCodes().find(code => code.code === e.target.value);
                              handleOwnerDeptChange(dept.id, 'deptCode', e.target.value);
                              handleOwnerDeptChange(dept.id, 'deptName', selectedDept?.codeName || '');
                            }}
                            disabled={mode === 'view'}
                            displayEmpty
                          >
                      ...
                      <TableCell>
                        <Box sx={{ display: 'flex', gap: 1 }}>
                          <TextField
                            ...
                            onChange={(e) => handleMeetingChange(meeting.id, 'meetingBodyName', e.target.value)}
                            disabled={mode === 'view'}
                      ...
                      <TableCell>
                        <FormControl fullWidth size="small">
                          <Select
                            ...
                            onChange={(e) => handleMeetingChange(meeting.id, 'memberGubun', e.target.value)}
                            disabled={mode === 'view'}
                            displayEmpty
                          >
                            <MenuItem value="">선택하세요</MenuItem>
