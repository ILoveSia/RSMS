...
import { TextField, DataGrid, Chip } from '@/shared/components/ui/data-display';
...
const AuditorAssignmentDialog: React.FC<AuditorAssignmentDialogProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* 선택된 점검자 정보 */}
      {selectedAuditor && (
        <Box sx={{ p: 2, backgroundColor: 'var(--bank-primary-bg)', borderRadius: 1, border: '1px solid var(--bank-primary)', mt: 2 }}>
      ...
      <Box sx={{ flex: 1, minHeight: 300, mt: 2 }}>
        <DataGrid
          data={auditorList}
          columns={[
            { field: 'empNo', headerName: '사번', width: 120 },
            { field: 'empName', headerName: '성명', width: 140 },
            { field: 'deptName', headerName: '부서', flex: 1, minWidth: 160 },
            { field: 'positionName', headerName: '직급', width: 120 },
          ] as DataGridColumn<AuditorInfo>[]}
          rowIdField={'empNo'}
          selectable
          multiSelect={false}
          ...
          onRowSelectionChange={(ids) => {
            ...
            setSelectedRows(ids.map(String));
            const found = auditorList.find(a => a.empNo === id) || null;
            setSelectedAuditor(found);
          }}
          onRowClick={(row) => handleAuditorSelection(row as AuditorInfo)}
          height={350}
        />
      </Box>
    </BaseDialog>
