 ...
 */
const NavigationMenu: React.FC<NavigationMenuProps> = ({ showHiddenRoutes = false }) => {
    ...
    return (
      <React.Fragment key={item.path}>
        <ListItem disablePadding sx={{ pl: depth * 2 }}>
          ...
          >
            <ListItemIcon sx={{ minWidth: 40 }}>{renderIcon(item.icon, depth > 0)}</ListItemIcon>
  ...
  return (
    <Box sx={{ width: '100%', maxWidth: 280 }}>
      ...
      <List component='nav' sx={{ py: 1 }}>
        {navigationMenu.filter(shouldShowItem).map((item: any) => renderMenuItem(item))}
      </List>
